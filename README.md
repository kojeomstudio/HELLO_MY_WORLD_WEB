...
- **Voxel World**: Procedurally generated 3D world with noise-based terrain, caves, ores, schematic-based trees (oak, pine, jungle, birch, cactus), rivers, multi-room dungeons, and 14 biomes (grassland, forest, desert, snow, taiga, jungle, savanna, mountains, swamp, ocean, plus ocean/underground variants) selected via heat/humidity noise
...
- **Smelting System**: 20 smelting recipes via furnace with fuel consumption, correct ore-to-ingot and food mappings, 23 fuel items with burn times loaded from `smelting.json`
...
- **Config-Driven Inventory**: Stack max size and chest inventory size configurable via `inventory` section in `server_config.json`
- **Config-Driven Knockback**: Knockback formula constants (base value, resistance, multipliers) configurable via `knockback` section in `server_config.json`
- **Config-Driven Weather Probabilities**: Weather probability tables for night/dusk-dawn/day configurable via `weatherProbabilities` section in `server_config.json`
- **Config-Driven Lighting**: Max light level and per-block emission levels configurable via `lighting` section in `server_config.json`
- **Config-Driven Crops**: Growth times, water proximity radius, growth multiplier configurable via `crops` section in `server_config.json`
- **Config-Driven Player Damage**: Lava/block damage intervals, food/saturation drain rates, health regen, drowning parameters configurable via `playerDamage` section in `server_config.json`
- **Config-Driven Gameplay**: Start items, experience formula, raycast distance, interact distance, broadcast range configurable via `gameplay` section in `server_config.json`
- **Config-Driven Dungeon**: Room size/count, corridor dimensions, chest/wall chances, Y range, spawn interval configurable via `dungeon` section in `server_config.json`
- **Config-Driven Explosions**: Block resistance defaults, drop chance, knockback multipliers configurable via `explosions` section in `server_config.json`
- **Config-Driven Tools**: Max wear (65536) configurable via `tools` section in `server_config.json`
- **Config-Driven World Persistence**: Max backups, max forceloaded chunks, emerge concurrency configurable via `worldPersistence` section in `server_config.json`
...
│   ├── biomes.json       # 14 biome definitions with tree types and decorations
...
- **World Generation**: 14 biomes (grassland, forest, desert, snow, taiga, jungle, savanna, mountains, swamp, ocean, plus ocean/underground variants) with heat/humidity noise, weighted biome selection with vertical blending, heat/humidity blend noise for smooth transitions, biome-specific cave liquids (water in jungle/swamp, lava in deep underground), dust nodes (snow in tundra/taiga/mountains), water top blocks (ice in cold biomes), river water/riverbed support, schematic-based trees (oak, pine, jungle, birch, cactus), river generation, 13 ore/mineral types with scatter, vein (3D noise contour), blob, puff, sheet, and stratum placement, multi-room dungeons with corridors and loot chests, cave systems with large caverns, random-walk tunnel generation with variable radius, biome-aware cave liquid filling. 9 map generators: Flat, Noise, V5, V6 (legacy with mud flow, 5 biome types, desert temples), V7 (default minetest terrain with dual-blend mountains/ridges/rivers), Valleys, Carpathian, Fractal, Singlenode
- **Security**: XSS-safe rendering (textContent/DOM APIs, CSS color validation, bgcolor validation), CORS-restricted origins with scoped headers/methods, CSP headers with nonce-based styles (no unsafe-eval, no unsafe-inline scripts/styles, frame-ancestors none, connect-src restricted to explicit WebSocket origins), HSTS, Cross-Origin-Opener-Policy, Cross-Origin-Resource-Policy, Referrer-Policy, Permissions-Policy, HTML entity encoding for chat (EnrichedString.toHtml escapes &, <, >, "), sign text sanitization, player name sanitization, IP-based rate limiting with join cooldown, per-account brute-force lockout (5 failed attempts -> 5-minute lockout, keyed by IP+name, atomic ConcurrentDictionary updates to prevent race conditions), IP connection rate limiting middleware (10 connections/minute per IP), PBKDF2 password hashing (configurable iterations via server_config.json, default 100k iterations, SHA256, random salt), constant-time comparison, privilege escalation protection (no self-grant/self-revoke/server privilege revocation), thread-safe privilege persistence, tiered privilege escalation protection, area protection with ownership checks (including TNT detonation), protection bypass privilege (configurable via protection.json), chest/furnace proximity validation, rollback recording, persistent ban database, block coordinate range validation, integrated AntiCheatValidator with per-player movement tracking (speed/fly/jump validation per physics mode, violation counting, position correction), input validation on /give (item whitelist) and /spawn (entity type whitelist) commands, rate limiting on all SignalR hub methods, configurable chat rate limit from server config, item ID validation on give commands, entity type validation on spawn commands, consistent password policy (8-char minimum, 128-char max), CI security scanning with fail-on-detect, npm audit in CI, minimal permissions for CI workflows, authenticated hub methods, explicit content-type whitelist for static files (no ServeUnknownFileTypes), SignalR max receive message size (128KB), runtime data files, particle spawner privilege check (interact required) with input validation (texture length, amount, expiration, size, NaN/Infinity bounds), mod channel message sanitization (HTML entity encoding before relay), GridCraft grid string length validation (128 chars per cell), profiler secret environment variable override (PROFILER_SECRET env var takes precedence over JSON config), fixed double HTML encoding bug (server-encodes chat, client no longer re-encodes)excluded from git, detached inventory creation restricted to server privilege, smelting item ID length validation, grid craft size validation, configurable Kestrel connection limit via MAX_CONNECTIONS env var, WORLD_SEED env var for deployment-safe seed configuration, thread-safe inventory operations, profiler endpoint secret-based authentication, EnrichedString color hex validation, client-side settings range clamping, generic client error messages (no internal details leaked)
- **CI**: GitHub Actions pipeline with Ubuntu server+client builds, NuGet caching, client typecheck+build, security scan (npm audit, .NET vulnerability check, hardcoded secret detection), data integrity verification (JSON parsing + required file checks + schema consistency with crafting recipe validation), protocol integration test (automated server startup + SignalR connection test), root dependency install for test scripts, fail-on-missing .gitignore patterns, pinned action SHAs for supply chain security
- **Environment Variables**: `WORLD_SEED` overrides server seed, `MAX_CONNECTIONS` sets Kestrel connection limit, `SERVER_URL` overrides test script target, `PROFILER_SECRET` overrides profiler authentication secret (takes precedence over server_config.json)

## Recent Improvements

- **CSP Hardening**: `connect-src` directive now uses explicit WebSocket origins derived from CORS config instead of bare `wss:` scheme, preventing unauthorized WebSocket connections to arbitrary hosts
- **Configurable PBKDF2**: Password hashing now uses the `auth.pbkdf2Iterations` value from `server_config.json` (previously the config value was ignored in favor of a hardcoded default)
- **Rate Limit Cleanup**: Stale connection rate and lockout entries are periodically purged (every 60 seconds) to prevent unbounded memory growth
- **Config-Driven Player Damage**: Food saturation drain, starvation damage, drowning breath drain/recovery, and block/lava damage intervals are now read from `playerDamage` section in `server_config.json` instead of being hardcoded
- **PlayerDamageSettings/GameplaySettings**: New configuration classes in `ServerConfig.cs` for externalized gameplay balance values
