...
public class MapgenValleys : IWorldGenerator
{
    public string Name => "valleys";
    ...
    private float _altitudeChill = 90f;
    private float _riverDepth = 4f;
    private float _riverSize = 0.08f;
    private float _valleyDepth = 0.4f;
    private float _valleyWidth = 0.3f;
    ...
    public void LoadBiomes(string dataPath)
    {
        ...
        var valleysPath = Path.Combine(dataPath, "mapgen_valleys.json");
        if (File.Exists(valleysPath))
        {
            var json = File.ReadAllText(valleysPath);
            using var doc = JsonDocument.Parse(json);
            var root = doc.RootElement;
            if (root.TryGetProperty("altitudeChill", out var ac))
                _altitudeChill = (float)ac.GetDouble();
            if (root.TryGetProperty("riverDepth", out var rd))
                _riverDepth = (float)rd.GetDouble();
            if (root.TryGetProperty("riverSize", out var rs))
                _riverSize = (float)rs.GetDouble();
            if (root.TryGetProperty("valleyDepth", out var vd))
                _valleyDepth = (float)vd.GetDouble();
            if (root.TryGetProperty("valleyWidth", out var vw))
                _valleyWidth = (float)vw.GetDouble();
        }
    }
    ...
    {
        var baseTerrain = PerlinNoise2D(x * 0.01f, z * 0.01f);
        var detail = PerlinNoise2D(x * 0.05f, z * 0.05f) * 0.3f;

        var valleyNoise = PerlinNoise2D(x * _valleyWidth + 7000, z * _valleyWidth + 7000);
        var valleyFactor = valleyNoise > _valleyDepth ? (valleyNoise - _valleyDepth) * 15f : 0f;

        var altitude = GroundBase + (int)((baseTerrain + detail) * TerrainHeight + valleyFactor);

        var chillNoise = PerlinNoise2D(x * 0.005f + 2000, z * 0.005f + 2000);
        if (chillNoise > 0.3f && altitude > WaterLevel)
            altitude += (int)((chillNoise - 0.3f) * 10f);
    ...
    private BiomeDefinition GetBiomeAt(int x, int y, int z, int surfaceHeight)
    {
        ...
        var altitude = surfaceHeight - WaterLevel;
        if (altitude > 20)
        {
            var chillFactor = Math.Min((altitude - 20) / 40f, 1f);
            heat -= chillFactor * _altitudeChill * 0.5f;
        }
    ...
    {
        var riverNoise = PerlinNoise2D(baseX * _riverSize + 3000, baseZ * _riverSize + 3000);
        var riverWidth = 0.15f;
        ...
        for (int x = 0; x < Chunk.Size; x++)
        {
            for (int z = 0; z < Chunk.Size; z++)
            {
                var worldX = baseX + x;
                var worldZ = baseZ + z;
                var absRiver = MathF.Abs(PerlinNoise2D(worldX * _riverSize + 3000, worldZ * _riverSize + 3000));

                if (absRiver < riverWidth)
                {
                    var surfaceY = heightMap[x, z];
                    var localSurfaceY = surfaceY - baseY;
                    var depth = (int)((riverWidth - absRiver) / riverWidth * _riverDepth);
                    ...
                    for (int dy = 0; dy <= depth; dy++)
                    {
                        ...
                        {
                            if (ly >= localSurfaceY - depth && blocks[x, ly, z] != (ushort)BlockType.Bedrock)
    ...
    {
        var rng = new Random(_seed + baseX * 1664525 + baseZ * 1013904223);
